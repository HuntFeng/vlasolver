...
int main(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (token == std::string("--filename"))
            filename = argv[++i];
        if (token == std::string("--max-subsp"))
            m = std::atoi(argv[++i]);
        if (token == std::string("--max-restarts"))
            cycLim = std::atoi(argv[++i]);
        if (token == std::string("--tol"))
            convTol = std::stod(argv[++i]);
        if (token == std::string("--ortho"))
            ortho = argv[++i];
        if (token == std::string("--rand_rhs"))
            rand_rhs = true;
        if (token == std::string("--help") || token == std::string("-h")) {
            std::cout
                << "Kokkos GMRES solver options:" << std::endl
                << "--filename    :  The name of a matrix market (.mtx) file "
                   "for "
                ...
                << std::endl
                << "--max-restarts:  Maximum number of GMRES restarts (Default "
                   "50)."
                << std::endl
                << "--tol         :  Convergence tolerance.  (Default 1e-10)."
                << std::endl
                << "--ortho       :  Type of orthogonalization. Use 'CGS2' or "
                   "'MGS'. "
                   "(Default 'CGS2')"
                << std::endl
                << "--rand_rhs    :  Generate a random right-hand side b.  "
                   "(Else, "
                   "default uses b = vector of ones.)"
                << std::endl
                << "--help  -h    :  Display this help message." << std::endl
                << "Example Call  :  ./Gmres.exe --filename Laplace3D100.mtx "
                   "--tol "
                   "1e-5 --max-subsp 100 "
                << std::endl
                << std::endl;
    ...
    Kokkos::initialize();
    {
        ...
        CRS A =
            KokkosSparse::Impl::read_kokkos_crst_matrix<CRS>(filename.c_str());
        ...
        // Get full gmres handle type using decltype. Deferencing a pointer
        // gives a reference, so we need to strip that too.
        using GMRESHandle =
        ...
        std::cout << "=========================================" << std::endl;
        std::cout << "Verify from main: Ending residual is " << endRes
                  << std::endl;
