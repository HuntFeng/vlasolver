...
with h5py.File(
    ...
    ax_profile[1].plot(
        x_f[G:-G], ni_exact[G:-G, (ny + 2 * G) // 2], "k-.", label=f"n={n}, y=0.25"
    )
...
for i, n in enumerate(n_range):
    ...
    errors_ni_mid[i] = np.linalg.norm(
        ni[G:-G, (ny + 2 * G) // 2] - ni_exact_mid, np.inf
    )
    ax_profile[1].plot(
        x[G:-G], ni[G:-G, (ny + 2 * G) // 2], "s-", label=f"n={n}, y=0.25"
    )
    # ni_exact_05 = interp_ni(
    #     (
    #         x[G:-G],
    #         0.25 * np.ones(nx),
    #     )
    # )
    # ni_05 = (ni[G:-G, (ny + 2 * G) // 2] + ni[G:-G, (ny + 2 * G) // 2 - 1]) / 2
    # errors_ni_05[i] = np.linalg.norm(
    #     ni_05[(nx + 2 * G) // 2 : -G] - ni_exact_05[(nx + 2 * G) // 2 : -G], np.inf
    # )
    # ax_profile[1].plot(x[G:-G], ni_05, "s-", label=f"n={n}, y=0.25")
...
plt.loglog(1 / n_range, 1 / n_range**3, "--", label="$O(h^3)$")
plt.xlabel("h")
plt.ylabel("err")
plt.legend()
