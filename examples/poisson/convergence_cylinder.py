...
with h5py.File(
    ...
    interp_phi = RegularGridInterpolator((x, y), phi_exact)
    interp_Ex = RegularGridInterpolator((x, y), Ex_exact)
    interp_Ey = RegularGridInterpolator((x, y), Ey_exact)

    X, Y = np.meshgrid(x, y, indexing="ij")
    fig, ax = plt.subplots(figsize=(6, 3))
    c = ax.contourf(X, Y, Ex_exact, cmap="jet", levels=50)
    plt.colorbar(c)
    plt.title("$E_x$")
    ax.set_xlim(0, 1)
    ax.set_ylim(0, 0.5)
    ...
    fig, ax = plt.subplots(figsize=(6, 3))
    c = ax.contourf(X, Y, phi_exact, cmap="jet", levels=50)
    plt.colorbar(c)
    plt.title("$\\phi$")
    ax.set_xlim(0, 1)
    ax.set_ylim(0, 0.5)
...
for i, n in enumerate(n_range):
    ...
    errors_u[i] = np.linalg.norm((phi[G:-G, G:-G] - u_exact).flat, np.inf)
