...
def _case3_geometry(direction: int, extra: int, i: int, j: int):
    ...
    # Extra interface point: interpolate from grid points one cell out.
    if extra == Direction.R:
...
def _build_local_system(eval_data, geom, row_ifaces, theta_inputs, betas):
    ...
    `geom`          : dict from `_caseN_geometry`, supplying per-interface n_x,
                      n_y, a, b, a_tau.
    `row_ifaces`    : 3-tuple of labels ('R', 'T', ..., 'extra') matching the
                      derivation file's points-list order.
    `theta_inputs`  : dict with keys 'R','T','L','B','r','t','l','b' (1.0 if
                      unused in this sub-case).
    `betas`         : (beta_p, beta_m, beta_jump) — physical, no swapping.
    """
...
def _solve_case3_local(sub_case, eta, direction, extra, geom):
    ...
    _eps_p, _eps_m, eps_jump, _, _ = geom["eps"]
    return _build_local_system(
        eval_data, geom, _CASE3_ROW_IFACES[sub_case],
        theta_inputs, (_eps_p, _eps_m, eps_jump),
    )
...
def _solve_case4_local(sub_case, eta, direction, geom):
    ...
    _eps_p, _eps_m, eps_jump, _, _ = geom["eps"]
    return _build_local_system(
        eval_data, geom, _CASE4_ROW_IFACES[sub_case],
        theta_inputs, (_eps_p, _eps_m, eps_jump),
    )
