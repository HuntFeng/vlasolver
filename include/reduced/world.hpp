...
struct World {
    ...
    Kokkos::View<double**> eta;        // level set field
    Kokkos::View<double**> eps_p;      // permittivity (eta > 0), need to fill the entire region for interpolation
    Kokkos::View<double**> eps_m;      // permittivity (eta < 0), need to fill the entire region for interpolation
    Kokkos::View<double**> jump_a;      // poisson jump condition a
    Kokkos::View<double**> jump_b;      // poisson jump condition b
    ...
    World(Grid& grid)
        ...
        Kokkos::deep_copy(eps_p, 0.0);
        Kokkos::deep_copy(eps_m, 0.0);
