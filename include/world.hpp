...
/**
 * Electron closure model selecting how the charge density is assembled.
 * - Kinetic:   every species (including electrons) is evolved kinetically and
 *              contributes q[sp] * n[sp] to the charge density.
 * - Boltzmann: only ions are evolved kinetically; electrons follow a Boltzmann
 *              distribution n_e = exp(phi), contributing -exp(phi).
 */
enum class ElectronModel { Kinetic, Boltzmann };

/**
 * World struct contains physical properties of the particles, fields, and the immersed boundary.
 * @tparam WorldType    the derived World type (CRTP)
 * @tparam N            number of kinetic species
 * @tparam EM           electron closure model (Kinetic or Boltzmann)
 */
template <typename WorldType, int N, ElectronModel EM>
struct World {
    static constexpr int nspecies             = N;  // number of kinetic species
    static constexpr ElectronModel electron_model = EM;
    ...
    // Per-species physical properties. Default to 1.0 so a single ion species
    // (Boltzmann model, normalized to ion quantities) keeps q/m = 1.
    Kokkos::Array<double, N> m;
    Kokkos::Array<double, N> q;
    Kokkos::Array<double, N> T;

    // Short species names used to build diagnostic dataset names ("n"+name, "f"+name).
    // e.g. {"i"} -> ni/fi, {"e","i"} -> ne/ni/fe/fi. Set by the derived World / driver.
    std::array<std::string, N> species_names;
    ...
    World(Grid<N>& grid)
        ...
        // Default per-species properties to 1.0 (normalized to electron
        // quantities). Drivers set the real values explicitly. Derived Worlds
        // that need physical m/q/T at construction time (e.g. to compute a wall
        // potential in a member initializer) should define their own explicit
        // constants rather than rely on these defaults.
