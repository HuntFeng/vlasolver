...
class Vlasolver {
  private:
    ...
    Kokkos::Array<double, 2> mu = {1.0, 1e6};    // mass ratio of the particle (relative to the electron mass)
    Kokkos::Array<double, 2> origin = {0.0, -6}; // origin of the grid
    Kokkos::Array<double, 2> size = {4 * Kokkos::numbers::pi, 12.0}; // size of the grid
    Kokkos::Array<int, 2> ncells = {32 + 2 * ngc, 64 + 2 * ngc}; // number of cells in the grid (including ghost cells)
     ...
     *
     * The coordinate indexes starts from 0 to ncells - 1.
     **/
    KOKKOS_FUNCTION
  ...
  public:
    ...
    void solve() {
        ...
        File file("data/strong_landau.hdf", HighFive::File::Overwrite);
